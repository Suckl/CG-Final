...
#include"micro_obj_loader.h"
#include "model.h"

Model::Model(const std::string& filepath) {
    microobj::attrib_t attrib;
    microobj::face_indices_t face;
    ...
    std::unordered_map<Vertex, uint32_t> uniqueVertices;
        ...
        for (const auto& faces : face) {
            Vertex vertex{};

            vertex.position.x = attrib.vertices[3 * faces.vertex_index + 0];
            vertex.position.y = attrib.vertices[3 * faces.vertex_index + 1];
            vertex.position.z = attrib.vertices[3 * faces.vertex_index + 2];

            if (faces.normal_index >= 0) {
                vertex.normal.x = attrib.normals[3 * faces.normal_index + 0];
                vertex.normal.y = attrib.normals[3 * faces.normal_index + 1];
                vertex.normal.z = attrib.normals[3 * faces.normal_index + 2];
            }

            if (faces.texcoord_index >= 0) {
                vertex.texCoord.x = attrib.texcoords[2 * faces.texcoord_index + 0];
                vertex.texCoord.y = attrib.texcoords[2 * faces.texcoord_index + 1];
            }
